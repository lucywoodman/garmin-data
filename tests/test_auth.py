...
from garmin_data.auth import TOKEN_DIR, create_client, login
...
class TestResumeSession:
    @patch("garmin_data.auth.TOKEN_DIR")
    @patch("garmin_data.auth.create_client")
    def test_resume_loads_tokens(self, mock_create, mock_token_dir):
        ...
        mock_client.login.assert_called_once_with(tokenstore=str(mock_token_dir))
        assert result is mock_client

    @patch("garmin_data.auth.TOKEN_DIR")
    def test_resume_returns_none_when_no_tokens(self, mock_token_dir):
        mock_token_dir.exists.return_value = False
